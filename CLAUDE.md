...
モデル名・ reasoning effort は `eliza/models.py` の定数（現行は全エージェント同一モデル `grok-4.7`、effort だけ用途別に変える）。
...
- ツールループの `sample` は `HEAVY_REASONING_EFFORT`（xhigh）。最終 `parse` だけ `LIGHT_REASONING_EFFORT`（low）
